...
test("policy defaults keep intervention experiments opt-in and preserve described checks", async (t) => {
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      compaction: { automatic: false, contextPercent: 90 },
    }),
  );
  const configured = await loadPolicy(root);
  assert.deepEqual(configured.compaction, {
    automatic: false,
    contextPercent: 90,
    cooldownTurns: 5,
  });
...
test("obsolete budgets and invalid execution-selection configuration fail closed", async (t) => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }));
  for (const policy of [
    { compaction: { automatic: "off" } },
    { compaction: { contextPercent: 0 } },
    { compaction: { contextPercent: 101 } },
    { compaction: { cooldownTurns: -1 } },
