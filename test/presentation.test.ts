...
function contains(text: string, values: string[]): void {
	for (const value of values) assert.ok(text.includes(value), `Missing ${value} in:\n${text}`);
}
...
test("long instructions remain complete and bounded state visibly reports omissions", () => {
	...
	const answers = Object.fromEntries(
		Array.from({ length: 9 }, (_, i) => [`q${i}`, { type: "noul" as const, noul: i / 10 }]),
	);
	contains(formatEvaluation({ ...evaluation, answers }), ["3 more answers"]);
	contains(formatDecisionDetails({ result: { ...evaluation, answers } }), ["q8", "0.8"]);
});
	...
	for (const [reason, expected] of cases) {
		for (const mode of ["steer", "shadow"] as const) {
			for (const expanded of [false, true]) {
				...
				if (reason !== "no-failures") assert.doesNotMatch(text, /no tool failures reported/i);
...
test("legacy recovery observations infer only batch-local absence of reported failures", () => {
	for (const batchFailures of [0, 1, undefined]) {
		for (const expanded of [false, true]) {
			const text = renderRecovery({ status: "observed", batchFailures }, expanded);
			assert.doesNotMatch(
				text,
				/\bsteer\b|\bshadow\b|assessment requested|in progress|assessment completed|healthy/i,
			);
			if (batchFailures === 0) assert.match(text, /no tool failures reported in this batch/i);
			else {
...
test("recovery presentation retains assessed outcomes and contains unsupported restored records", () => {
	for (const mode of ["steer", "shadow"]) {
		for (const action of ["none", "replan", "ask-user"]) {
			...
			contains(expanded, [evaluation.model, "0.8123456789012345", `Action: ${action}`]);
...
test("historical receipts remain visible and source-free for every purpose and outcome", () => {
	type Renderer = Parameters<ExtensionAPI["registerEntryRenderer"]>[1];
	const entries = new Map<string, Renderer>();
	registerPresentation({
		registerEntryRenderer: (name: string, renderer: Renderer) => entries.set(name, renderer),
		registerMessageRenderer() {},
	} as unknown as ExtensionAPI);
	const theme = {
		fg: (_color: string, text: string) => text,
	} as Parameters<Renderer>[2];
	for (const purpose of ["Decision", "Compaction", "Historical purpose"]) {
		for (const status of ["completed", "failed", "cancelled"] as const) {
			for (const expanded of [false, true]) {
				const outcome = entries.get("jevons.receipt")!(
					{
						data: { ...receipt, purpose, status, source: "PRIVATE_SOURCE" },
					} as Parameters<Renderer>[0],
					{ expanded } as Parameters<Renderer>[1],
					theme,
				);
				assert.ok(outcome);
				const shown = outcome.render(200).join("\n");
				contains(shown, [purpose, status]);
				if (expanded) contains(shown, [receipt.model, "Raw answers"]);
...
test("registered expanded renderers wrap at narrow widths and activity receipts stay source-free", () => {
	...
	registerPresentation({
		registerEntryRenderer: (name: string, renderer: EntryRenderer) => entries.set(name, renderer),
	...
	const autopilot = {
		...
		details: {
			kind: "autopilot",
			coverage: "13 candidates were not assessed: 2 explicit-only, 11 request limits.",
			task: "SECRET_SOURCE",
		},
	} as Parameters<MessageRenderer>[0];
	const compactPlan = messages.get("jevons")!(autopilot, { ...options, expanded: false }, theme)!;
	...
	contains(planText, ["31/44", "13 candidates", "explicit-only", "request limits"]);
	...
	for (const [name, data] of [
		["jevons.receipt", { purpose: "Decision", request: { state: "SECRET_SOURCE" } }],
		[
			"jevons.recovery",
			{
				status: "assessed",
