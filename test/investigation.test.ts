...
async function fixture() {
  ...
  return {
    ...
    messages: () =>
      sent.map(([message]) => ({ ...message, role: "custom", timestamp: 1 })),
  };
}
...
test("reuses exactly one reviewed patch in one turn; source-free cap survives policy changes and reload", async () => {
  ...
  assert.equal(
    evidence.otherFindingsNotInvestigated,
    h.report.findings.length - 1,
  );
  const checkpoints = h.manager
    .getEntries()
  ...
  assert.equal(h.sent.length, 1);
  assert.equal(
    (await h.emit("context", { messages: h.messages() }))!.messages.length,
    0,
  );
...
test("explicit enable, trust, idle, task completeness and threshold gate investigation", async () => {
  ...
  assert.equal(
    h.manager.getEntries().filter((entry) => entry.type === "custom").length,
    0,
  );
});
...
test("incomplete reports and remote reports never trigger or reserve investigation", async () => {
  for (const change of ["partial", "omitted", "remote"] as const) {
    ...
    if (change === "remote")
      h.report.comparison = "github owner/repo#1 base...head";
    await h.run(async () => {
...
test("unavailable recheck is source-free and cancellation stops waiting for an uncooperative callback", async () => {
  ...
  const entries = failed.manager
    .getEntries()
    .filter((entry) => entry.type === "custom");
  assert.equal(entries.length, 2);
  assert.equal(
    (entries.at(-1) as { data: { status: string } }).data.status,
    "unavailable",
  );
