...
async function fixture(t: TestContext, repository = true) {
	...
	const jj = async (...args: string[]) => {
		const result = await exec("jj", ["--no-pager", "--color", "never", ...args], { cwd: root });
	...
	let assess: Evaluate = async (request) => ({
		...
		answers: Object.fromEntries(
			Object.keys(request.questions).map((id) => [id, { type: "noul", noul: 0.01 }]),
		),
	});
...
test("mandatory defaults cannot be deselected and failure does not skip remaining selected checks", async (t) => {
	...
	assert.deepEqual(await h.executed(), ["default-mandatory", "explicit-mandatory"]);
	assert.deepEqual(
...
test("failed optional judgment retains all checks and separate confirmation still permits execution", async (t) => {
	const h = await fixture(t);
	h.runtime.policy!.checks = [h.check("mandatory"), h.check("optional", { mandatory: false })];
	h.assess(async () => {
		h.runtime.active = false;
...
for (const change of ["workspace", "parent", "task", "policy", "session", "pause"] as const) {
	test(`${change} change during confirmation rejects execution`, async (t) => {
		const h = await fixture(t);
		h.confirm(async () => {
			if (change === "workspace")
				await writeFile(join(h.root, "unrelated.ts"), "export const changed = true;\n");
			if (change === "parent") await h.jj("new", "-m", "Same files, new parent");
			if (change === "task") h.runtime.deliveredUser("Do not run old checks");
			if (change === "policy") h.runtime.policy = structuredClone(h.runtime.policy!);
...
test("pause kills an executing check and prevents delayed effects or later checks", {
	timeout: 5000,
}, async (t) => {
	const h = await fixture(t);
	h.runtime.policy!.checks = [
	...
	assert.deepEqual(await h.executed(), ["running"]);
	assert.equal(await readFile(join(h.root, "source.ts"), "utf8"), "export const value = 1;\n");
});
