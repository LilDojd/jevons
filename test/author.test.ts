...
const failed = (error: Error) =>
	/Prompt author failed/.test(error.message) && !/PRIVATE|sk-/.test(error.message);
	...
	const badQuestions: unknown[] = [
		...
		},
		{
			q: {
				type: "choice",
				instructions: "Question",
				criteria: Object.fromEntries(Array.from({ length: 33 }, (_, i) => [`c${i}`, "Option"])),
			},
		},
		...
		Object.fromEntries(Array.from({ length: 33 }, (_, i) => [`q${i}`, questions.relevant])),
		Object.fromEntries(
...
test("bounds input before completion and bounds questions together with unchanged state", async () => {
	...
	for (const value of [
		...
		...[cycle, deep, NaN, 1, true, "x".repeat(48_001), `sk-${"a".repeat(25)}`].map((state) => ({
			...input,
			state,
		})),
	]) {
	...
	for (const stage of ["payload", "return"]) {
		for (const kind of ["scope", "available"]) {
			...
			h.complete(async (_model, _context, options) => {
				...
				if (stage === "payload") await options!.onPayload!({}, h.writer as never);
				return response();
			});
			await assert.rejects(h.run(), failed);
			assert.equal(h.calls.length, 1);
	...
	for (const kind of ["lifetime", "permission"]) {
		...
		await Promise.race([
			...
			new Promise<never>((_, reject) => {
				const timer = setTimeout(() => reject(new Error("Late response body not cancelled")), 1000);
