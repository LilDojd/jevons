...
function assertRanges(patch: string, chunk: ReturnType<typeof parseDiff>["chunks"][number]) {
	...
	assert.equal(chunk.added, hunk.lines.filter((line) => line.startsWith("+")).length);
	assert.equal(chunk.deleted, hunk.lines.filter((line) => line.startsWith("-")).length);
...
test("parses additions, deletions, renames, mode changes and quoted paths without losing headings or EOF markers", () => {
	...
	const snapshot = parseDiff(modified + addition + deletion + rename + mode + quoted, "base..head");
	...
	for (const i of [0, 1, 2, 5]) assertRanges(snapshot.chunks[i]!.patch, snapshot.chunks[i]!);
});
...
test("fingerprints exact selected patch bytes and revision identity, while filtering literal paths on both sides of renames", () => {
	...
	assert.equal(
		first.fingerprint,
		parseDiff(a + b.replace("after", "changed"), "revision-one", ["src"]).fingerprint,
	);
	assert.notEqual(first.fingerprint, parseDiff(a + b, "revision-two", ["src"]).fingerprint);
	assert.notEqual(
		first.fingerprint,
		parseDiff(a.replace("@@\n", "@@ heading\n") + b, "revision-one", ["src"]).fingerprint,
	);
...
test("chunks large UTF-8 hunks with exact ranges and complete changed-row coverage", () => {
	const old = Array.from({ length: 1000 }, (_, i) => `old-${i} ${"λ".repeat(30)}\n`).join("");
	const next = Array.from({ length: 1000 }, (_, i) => `new-${i} ${"語".repeat(30)}\n`).join("");
...
test("keeps small replacement runs intact and overlaps available unchanged context", () => {
	const context = Array.from({ length: 150 }, (_, i) => ` unchanged-${i} ${"x".repeat(60)}`);
	const rows = [...context.slice(0, 100), "-before", "+after", ...context.slice(100)];
	const raw = `--- a/a.ts\n+++ b/a.ts\n@@ -1,151 +1,151 @@ heading\n${rows.join("\n")}\n`;
	const snapshot = parseDiff(raw, "r");
	assert.deepEqual(snapshot.omitted, []);
	assert.ok(snapshot.chunks.length > 1);
	assert.ok(snapshot.chunks.some((chunk) => chunk.patch.includes("-before\n+after\n")));
	for (const chunk of snapshot.chunks) assertRanges(chunk.patch, chunk);
	for (let i = 1; i < snapshot.chunks.length; i++) {
		const previous = parsePatch(snapshot.chunks[i - 1]!.patch)[0]!.hunks[0]!.lines;
...
test("omits sensitive old and new paths, binary changes, malformed patches and unsupported giant rows explicitly", () => {
	const rename = "diff --git a/.env b/public.txt\nrename from .env\nrename to public.txt\n";
	const binary =
	...
	assert.throws(() => parseDiff("x".repeat(4 * 1024 * 1024 + 1), "r"), /oversized/);
});
...
test("local collection prefers jj, snapshots current content, scopes subdirectories, and flags Git untracked files", async (t) => {
	const patch = gitPatch("src/a.ts", "old\n", "new\n") + gitPatch("b.ts", "old\n", "new\n");
	...
	assert.doesNotMatch(await fs.readFile(fixture.calls, "utf8"), /"binary":"git"/);
...
test("real jj selected-path fingerprints ignore unrelated working-copy edits", async (t) => {
	...
	await fs.writeFile(join(root, "unrelated.ts"), "export const unrelated = 1;\n");
	const before = await collectLocalDiff(root, ["selected.ts"]);
	await fs.writeFile(join(root, "unrelated.ts"), "export const unrelated = 2;\n");
	...
	assert.notEqual((await collectLocalDiff(root, ["selected.ts"])).fingerprint, before.fingerprint);
});
...
for (const vcs of ["jj", "git"] as const) {
	test(`real ${vcs} scopes collection before byte limits and treats metacharacters literally`, async (t) => {
		...
		for (const name of names) await fs.writeFile(join(root, name), "selected\n");
		await fs.writeFile(join(root, "large.ts"), "unrelated line\n".repeat(400000));
		if (vcs === "jj") await run(["--config", "snapshot.max-new-file-size=8000000", "status"]);
		else await run(["add", "."]);
...
test("local collection stops at a nested Git repository rather than selecting outer Jujutsu", async (t) => {
	...
	assert.equal(snapshot.comparison, `git ${nested} ${"a".repeat(40)}..working-copy`);
