...
test("SDK request retains actual tokens and model without provider extras or spending caps", async () => {
	...
	assert.ok(receipts.every((r) => r.accounting === "reported" && r.elapsedMs >= 0));
...
test("network failures count unknown usage without retries or blocking later explicit calls", async () => {
	...
	assert.ok(receipts.every((r) => r.accounting === "unknown" && r.status === "failed"));
	assert.deepEqual(summarizeUsage(receipts), {
		input: 0,
		output: 0,
		calls: 2,
...
test("pre-cancelled requests never dispatch; uncooperative transport cancellation has unknown usage", {
	timeout: 3000,
}, async () => {
	let calls = 0;
...
test("malformed answers retain valid reported usage and sanitize malformed model metadata", async () => {
	...
	for (const _body of bodies) await assert.rejects(jev.evaluate("Review", request), /failed/);
	assert.equal(calls, 3);
	assert.deepEqual(summarizeUsage(receipts), {
		input: 30,
		output: 6,
	...
	assert.ok(
		receipts.every((r) => typeof r.model === "string" && !r.answers && r.accounting === "reported"),
	);
...
test("missing, negative, fractional and overflowing usage remains unknown", async () => {
	...
	let calls = 0;
	const { jev, receipts } = fixture(async () => Response.json(bodies[calls++]));
	for (const _body of bodies) await assert.rejects(jev.evaluate("Review", request));
...
test("SDK timeout releases caller even when transport ignores cancellation", {
	timeout: 15000,
}, async () => {
	let calls = 0;
	const { jev, receipts } = fixture(async () => {
