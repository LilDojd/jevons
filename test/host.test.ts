...
test("real Node 24 Pi loads the extension and handles paused commands without network access", {
	timeout: 30_000,
}, async () => {
	...
	const child = spawn(
		...
		[
			"--import",
			preload,
			join(project, "node_modules/@earendil-works/pi-coding-agent/dist/bundle/cli.js"),
			"--mode",
			"rpc",
			"--no-session",
			"--approve",
				...
				if (event.type === "extension_ui_request" && event.method === "editor") {
					...
					child.stdin.write(
						`${JSON.stringify({
							...
							id: event.id,
							...(cancelEditor ? { cancelled: true } : { value: JSON.stringify(policy) }),
						})}\n`,
					);
				}
			} catch {
	...
	try {
		...
		assert.ok(!entries.data?.entries?.some((entry) => entry.customType === "jevons.receipt"));
		const saved = await readFile(join(state, "jevons.json"), "utf8");
		assert.equal(JSON.parse(saved).autopilot.skills, false);
		cancelEditor = true;
		await command({ type: "prompt", message: "/jevons settings" });
		const editor = events.findLast((event) => event.method === "editor");
		assert.equal(JSON.parse(editor!.prefill!).autopilot.skills, false);
		assert.equal(await readFile(join(state, "jevons.json"), "utf8"), saved);
		...
		await command({ type: "prompt", message: "/jevons pause" });
		await command({ type: "prompt", message: "/jevons settings" });
		const newSessionEditor = events.findLast((event) => event.method === "editor");
		assert.equal(JSON.parse(newSessionEditor!.prefill!).autopilot.skills, false);
		assert.equal(await readFile(join(state, "jevons.json"), "utf8"), saved);
		assert.equal(
