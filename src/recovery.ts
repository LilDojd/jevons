...
const questions: Request["questions"] = {
  ...
  category: {
    ...
    criteria: {
      ...
      insufficient:
        "No supported single category, conflicting causes, or insufficient evidence.",
    },
  },
  userOnly: {
    type: "noul",
...
export class RecoveryWindow {
  ...
  call(id: string, tool: string, input: Json, batch: number): void {
    ...
    this.calls.set(idHash, {
      ...
      identityOmittedBytes:
        (idBytes <= 512 ? 0 : idBytes) + (toolBytes <= 128 ? 0 : toolBytes),
      batch,
    ...
    if (this.calls.size > 4) {
      const oldest = this.calls.values().next().value!;
      this.droppedCalls++;
      this.droppedBytes +=
        oldest.input.bytes +
        (oldest.output?.bytes ?? 0) +
        oldest.identityOmittedBytes;
  ...
  result(id: string, output: Json, isError: boolean, final = false): void {
    ...
    call.output = field(
      output,
      !isError ? "success" : hasImage ? "image" : "none",
    );
  ...
  snapshot() {
    const calls = [...this.calls.values()].map((call) => ({
      ...call,
      repeats: [...this.calls.values()].filter(
        (other) => other.hash === call.hash,
      ).length,
...
export async function assessRecovery(
  ...
  if (
    ...
    !["invocation", "environment", "implementation", "insufficient"].includes(
      category.choice,
    )
  )
    ...
    action:
      user.noul >= policy.userConcern && user.noul > 0.5
        ...
        : retry.noul >= policy.retryConcern &&
          ...
          ? "replan"
          : "none",
    baseline: window.calls.some((call) => call.failedRepeats >= 2)
      ? "replan"
      : "none",
    evaluation,
  };
