...
export interface CheckResult {
	...
	termination: "exit" | "timeout" | "cancelled" | "output-limit" | "spawn-error";
	omittedBytes: number;
}
...
export async function runChecks(
	...
	for (const check of configured) {
		...
		if (!executable) throw new Error("Invalid executable check.");
		...
		const result = await new Promise<CheckResult>((resolve) => {
			...
			const finish = (code: number | null, termination: CheckResult["termination"] = "exit") => {
				if (finished) return;
				finished = true;
				clearTimeout(timer);
			...
			for (const stream of [child.stdout, child.stderr])
				stream.on("data", (chunk: Buffer) => {
					...
					let start = 0;
					while (start < tail.length && (tail.readUInt8(start) & 0xc0) === 0x80) start++;
