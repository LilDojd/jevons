import type { CheckConfig, Evaluate, Evaluation, Policy, Request } from "./contracts.ts";
...
export async function selectVerification(
	...
	const selection = (index: number) => {
		...
		if (!item) throw new Error("Missing verification selection.");
		return item;
	};
	const finish = () => {
		report.selected = configured.filter((_, i) => selection(i).selected);
	...
	const omit = (index: number, reason: string) => {
		selection(index).reason = reason;
		report.omitted.push(`Check ${index}: ${reason}; included without assessment.`);
	};
	...
	else if (!context.task.trim() && !context.diff.trim()) unavailable = "empty-context";
	else if (Buffer.byteLength(context.task) > 8_000 || Buffer.byteLength(context.diff) > 24_000)
	...
	for (const { index } of candidates) {
		...
		if (!check) throw new Error("Missing configured check.");
	...
	try {
		...
		for (const index of assessed) {
			const key = `check${index}`;
			const answer = Object.hasOwn(evaluation.answers, key) ? evaluation.answers[key] : undefined;
		...
		for (const index of assessed)
			omit(index, signal?.aborted ? "selection-cancelled" : "evaluation-unavailable");
	} finally {
		clearTimeout(timer);
