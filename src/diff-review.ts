...
export interface DiffReport {
	...
	questionMaps: Record<string, { chunkId: string; path: string; rule: string }>[];
}
	...
	for (const item of items) {
		...
		questions[`${chunkKey}_${ruleKey}`] = {
			...
			criteria: {
				...
				false: "No concrete concern under this rule is supported by this change.",
			},
		};
	}
...
function fits(request: Request): boolean {
	...
	return (
		...
		Buffer.byteLength(JSON.stringify(request.state)) +
			Math.max(...questions.map((question) => Buffer.byteLength(JSON.stringify(question)))) <=
			CONTEXT_BYTES
	);
}
...
function evaluationFor(raw: Evaluation, request: Request): Evaluation {
	if (
		...
		raw.elapsedMs < 0 ||
		Object.keys(raw.answers).some((key) => !Object.hasOwn(request.questions, key))
	)
		throw new Error("Invalid evaluation");
	const answers: Evaluation["answers"] = {};
	for (const key of Object.keys(request.questions)) {
		const answer = Object.hasOwn(raw.answers, key) ? raw.answers[key] : undefined;
...
export async function reviewDiff(
	...
	if (
		...
		rules.some((rule) => !rule.id || !rule.instructions.trim() || rule.instructions.length > 4000)
	) {
	...
	const run = async (items: Item[]) => {
		try {
			...
			for (const { chunk, chunkIndex, rule, ruleIndex } of items) {
				...
				if (answer?.type !== "noul") {
					report.omitted.push(`${chunk.path} [${chunk.id}]: missing or invalid ${rule.id} answer.`);
					continue;
				}
	...
	outer: for (const [chunkIndex, chunk] of chunks.entries()) {
		if (bounded.aborted) {
			report.omitted.push("Review cancelled or deadline exceeded; remaining chunks not assessed.");
			break;
		}
		...
		for (const [ruleIndex, rule] of rules.entries()) {
			...
			if (batch.length && !fits(requestFor(report.comparison, [...batch, item]))) {
				await run(batch);
				batch = [];
	...
	report.complete =
		report.reviewedChunks === report.totalChunks && !report.omitted.length && !bounded.aborted;
...
export function formatDiffReview(report: DiffReport): string {
	const safe = (text: string) =>
		text.replace(/[\p{Cc}\u202a-\u202e\u2066-\u2069]/gu, "?").slice(0, 300);
	...
	return [
		...
		...(report.findings.length > 20 ? [`${report.findings.length - 20} more findings`] : []),
		...report.omitted.slice(0, 10).map(safe),
		...(report.omitted.length > 10 ? [`${report.omitted.length - 10} more omissions`] : []),
	].join("\n");
}
