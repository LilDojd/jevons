...
const parameters = Type.Object({ questions: questionsSchema }, { additionalProperties: false });
const tool = {
...
export async function authorQuestions(
	...
	try {
		...
		if (typeof input.prompt !== "string" || !input.prompt.trim()) throw new Error(FAILURE);
		const request = parseRequest({
			state: input.state,
			questions: { prompt: { type: "noul", instructions: input.prompt } },
		});
		const prompt = request.questions.prompt;
		if (!prompt) throw new Error(FAILURE);
			...
			if (
				...
				(ctx.scopedModels.length &&
					!ctx.scopedModels.some(
						(item) => item.model.provider === provider && item.model.id === writerModel,
					))
			)
		...
		check();
		if (Buffer.byteLength(JSON.stringify(response.content)) > MAX_BYTES) throw new Error(FAILURE);
