...
import { parseRequest, questionsSchema, requestSchema } from "./schema.ts";
import { openSettings } from "./settings.ts";
import { formatVerification, verifyConfigured } from "./verify.ts";
		...
		async execute(id, params, signal, onUpdate, ctx) {
			...
			if (params.prompt) {
				const selected =
					...
					(ctx.model ? { provider: ctx.model.provider, model: ctx.model.id } : undefined);
				if (!selected) throw new Error("Select a prompt writer in jevons.json.");
				onUpdate?.({
					content: [
						{
							type: "text",
				...
				formatRestoredDetails(() => {
					...
					const text = details?.result
						...
						: result.content
								...
								.map((part) => part.text)
								.join("\n");
					return safeText(expanded && details ? formatDecisionDetails(details) : text);
				}),
				0,
				0,
			);
			...
			try {
				...
				if (action === "activity") {
					const receipts = ctx.sessionManager
						.getEntries()
						.filter((entry) => entry.type === "custom" && entry.customType === "jevons.receipt")
						.reverse();
					...
					if (page > pages)
						throw new Error(`Only ${pages} activity pages. Use /jevons activity 1 for the latest.`);
					show(
				...
				if (action === "ask") {
					...
					const prompt =
						args.join(" ") ||
						(ctx.hasUI
							? await ctx.ui.input("Ask Jev", "A focused question about explicit context")
							: undefined);
					if (!prompt || lifetime.aborted) return;
					const context = ctx.hasUI ? await ctx.ui.editor("Context for Jev", "") : undefined;
				...
				show(error instanceof Error ? error.message : "Jevons could not complete.");
			}
		},
	});
}
