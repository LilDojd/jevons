...
import { assessRecovery, RecoveryWindow, recoveryHash, recoveryMessages } from "../src/recovery.ts";
...
export function registerRecovery(pi: ExtensionAPI, runtime: Runtime): void {
	...
	function restore(ctx: ExtensionContext): void {
		...
		for (const entry of ctx.sessionManager.getEntries()) {
			if (entry.type !== "custom" || entry.customType !== checkpointType) continue;
			const data = entry.data as Record<string, unknown> | undefined;
			if (data?.version !== 1) continue;
			const count = (name: string, fallback = 0) =>
				typeof data[name] === "number" && Number.isSafeInteger(data[name]) && data[name] >= 0
					? data[name]
					: fallback;
			turn = Math.max(turn, count("turn"));
			interventions = Math.max(interventions, count("interventions"));
			shadowInterventions = Math.max(shadowInterventions, count("shadowInterventions"));
			lastIntervention = Math.max(lastIntervention, count("lastIntervention", -1));
	...
	function enabled(): boolean {
		return runtime.active && !!runtime.policy && runtime.policy.recovery.mode !== "off";
	}
	...
	pi.on("context", (event, ctx) => ({
		messages: event.messages.filter((message) => {
			if (message.role !== "custom" || message.customType !== "jevons.recovery") return true;
	...
	pi.on("tool_call", (event, ctx) => {
		...
		pending?.abort();
		observe(() => window.call(event.toolCallId, event.toolName, event.input as Json, turn));
	});
	...
	pi.on("turn_end", async (event, ctx) => {
		sync(ctx);
		if (!enabled()) return;
		const results = event.toolResults.filter((result) => !result.toolName.startsWith("jevons_"));
		...
		if (!results.length || unsupportedBatch || processedRevision === window.revision) return;
