...
async function rechecked(recheck: () => Promise<DiffSnapshot>, signal: AbortSignal) {
...
export function registerInvestigation(pi: ExtensionAPI, runtime: Runtime) {
		...
		return {
			messages: event.messages.filter(
				(message) =>
					message.role !== "custom" ||
					message.customType !== messageType ||
					(keep && (message.details as { id?: unknown } | undefined)?.id === current?.id),
			),
		};
	});
	...
	return async function investigate(
		...
		if (!policy) return;
		const concern = selectConcern(report, policy.review.investigateConcern);
		if (
			!concern ||
			ctx.sessionManager
				.getEntries()
				.some(
					(entry) =>
						...
						(entry.data as { fingerprint?: unknown } | undefined)?.fingerprint ===
							report.fingerprint,
				)
		)
			return;
		...
		try {
			...
			const fresh = (current: ExtensionContext) =>
				...
				cwd === current.cwd &&
				(anchor === null ||
					current.sessionManager.getBranch().some((entry) => entry.id === anchor));
			...
			const chunks = snapshot.chunks.filter(
				(chunk) => chunk.id === concern.finding.id && chunk.path === concern.finding.path,
			);
			const chunk = chunks[0];
			if (chunks.length !== 1 || !chunk?.patch || Buffer.byteLength(chunk.patch) > 8000) return;
			const id = randomUUID();
			...
			if (version === generation && session === ctx.sessionManager.getSessionId())
				pi.appendEntry(messageType, {
					status: signal.aborted ? "cancelled" : "unavailable",
					fingerprint,
					omission: "No investigation delivered; fresh initial diff evidence was unavailable.",
				});
		} finally {
			busy = false;
		}
